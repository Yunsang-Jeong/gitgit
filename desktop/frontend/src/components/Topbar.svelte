...
<header class="topbar">
  ...
  <nav class="topbar-view-switcher" aria-label="Repository views">
    <button class:selected={view === 'commit'} type="button" on:click={() => onViewChange('commit')} disabled={editMode}>Commit</button>
    <button class:selected={view === 'worktrees'} type="button" on:click={() => onViewChange('worktrees')} disabled={editMode}>Worktree</button>
    <button class:selected={view === 'search'} type="button" on:click={() => onViewChange('search')} disabled={editMode}>Search</button>
  </nav>
  ...
  {#if view !== 'search'}
    <ProjectSwitcher
      {projects}
      {activeProjectRoot}
      disabled={transitioning || editMode}
      showFavorites={false}
  ...
  <button class="toolbar-action remote-sync-action" class:spinning={syncing} type="button" on:click={onSync} disabled={view !== 'commit' || !repository || syncing || pulling || refreshing || transitioning || editMode} title={editMode ? 'Exit Edit Mode before changing repository state' : view === 'commit' ? 'Fetch all remotes and prune deleted refs' : 'Sync is available in Commit'}>
  ...
  <button class="toolbar-action remote-pull-action" class:spinning={pulling} type="button" on:click={onPull} disabled={view !== 'commit' || !repository || pulling || syncing || refreshing || transitioning || editMode} title={editMode ? 'Exit Edit Mode before changing repository state' : view === 'commit' ? 'Fast-forward the current tracking branch' : 'Pull is available in Commit'}>
  ...
  <button class="toolbar-action" class:spinning={refreshing} type="button" on:click={onRefresh} disabled={view === 'search' || !repository || refreshing || syncing || pulling || transitioning || editMode} title={editMode ? 'Exit Edit Mode before changing repository state' : view === 'search' ? 'Search sessions keep their own results' : 'Refresh repository state'}>
