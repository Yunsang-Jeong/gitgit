...
Search 화면에 들어가는 것만으로 session을 만들지 않는다. 처음에는 empty state를 표시하며 좌측 sidebar 또는 본문의 `+` 버튼을 눌렀을 때만 session을 만든다. Session alias는 sidebar에서 바로 수정할 수 있고 `×`로 삭제한다. Sidebar는 project, query, status와 실행 시각을 한눈에 비교할 수 있도록 유지하되 결과 조사 공간을 침범하지 않도록 compact width를 사용한다.
...
첫 condition 뒤의 각 condition에는 `AND` 또는 `OR`를 지정한다. Group은 괄호 개수를 각 행에서 직접 고르는 방식이 아니라, 서로 인접한 condition을 checkbox로 선택한 뒤 `Group selected`를 누르는 visual builder로 만든다. 이미 묶인 범위를 선택해 `Ungroup`을 누르면 바깥 group 한 겹을 제거한다. Condition 아래의 Expression strip은 생성된 query와 괄호 우선순위를 즉시 보여준다.
...
Query input 자체는 즉시 session에 반영되지만 기존 result는 자동으로 바꾸지 않는다. 마지막 성공 결과와 현재 query가 달라지면 `Changed · Run Search to update results`로 표시한다. 이전의 `Draft` 표현은 사용하지 않는다.
...
- Branch/ref 정보는 Commit column이 아니라 Message 아래의 작은 badge로 표시
