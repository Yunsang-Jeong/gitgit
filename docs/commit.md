---
title: Commit Module
description: Commit history 탐색, Filter, Inspector와 commit rewrite의 동작 및 안전 조건
audience:
  - human
  - ai-agent
status: active
...
```text
Commits  [Worktree]  [Branch]  [Edit commits]  [+ Filter]
```
...
Commit table은 graph, short SHA, subject, ref badge, author, date를 표시한다. 행을 선택하면 Inspector가 다음 정보를 제공한다.
...
- Commit message, author, file path를 Filter 또는 새 Search session으로 보내는 context action
...
Filter는 Commit module에만 적용된다. Search result에는 Commit Filter가 암묵적으로 적용되지 않는다.
...
Action은 `Highlight`, `Hide`, `Show`다. Show rule과 Hide rule은 각각 AND/OR 조합 방식을 선택할 수 있다. `My Jobs`, `3 Days` 같은 preset은 Settings에서 편집하며 `$me`와 `last:3d` 같은 값을 사용할 수 있다.
